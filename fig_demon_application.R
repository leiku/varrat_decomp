...
text(0.6,max(res.ts$com)+0.05*diff(range(res.ts$com)),
...
mtext("(d)", side=3, cex=1.1, line=-1.2, adj=0.05)
...
tiff("Figs/fig4_demon_application2.tif", 
     width=11, height=3, units="in",res=600,compression = "lzw")
op2<-par(mfrow=c(1,4),oma=c(0.5,0.5,0.5,0.5), mar=c(4,4.5,3,1),mgp=c(2,0.5,0),
        tck=-.02,cex.axis=1.4,cex.lab=1.6,cex.main=1.5)
...
text(0.7,max(res$vr[i.chosen, 1:2],na.rm=T)+0.15,
     ...
     adj=c(-0.1,0.5),cex=1.4)
mtext("(a)", side=3, cex=1.1, line=-1.2, adj=0.05)
...
text(0.65,max(res$comnull[i.chosen, 1:2])+0.02*diff(range(res$comnull[i.chosen, 1:2])),
     labels=bquote(paste(CV[com_ip]^2,"=",.(round(CV2comnull.class,4)))),
     adj=c(-0.05,0), cex=1.4)
mtext("(b)", side=3, cex=1.1, line=-1.2, adj=0.05)
...
text(0.65,max(res$comnull[i.chosen, 1:2])+0.02*diff(range(res$comnull[i.chosen, 1:2])),
     ...
     adj=c(-0.1,0), cex=1.4)
mtext("(c)", side=3, cex=1.1, line=-1.2, adj=0.05)
